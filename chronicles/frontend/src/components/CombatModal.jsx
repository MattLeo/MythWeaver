...
.turn-divider { color: #2a2d44; font-size: .8rem; flex-shrink: 0; }
...
.enemy-row {
  ...
  align-items: flex-end; flex: 1; min-height: 0;
  padding-bottom: .5rem;
}
...
.enemy-card.attack-out {
  animation: attack-forward .3s ease forwards;
}
...
.combatant-class {
  font-size: .58rem; color: #3a3d55; margin-top: .15rem;
...
.skill-btn:hover:not(:disabled) {
  background: rgba(200,150,42,.06); color: var(--goldl);
}
...
.dice-advantage-note {
  font-size: .65rem; color: var(--dim); font-style: italic;
}
...
.ds-pip {
  ...
  border: 1px solid #2a2d44; background: #0b0c15;
  transition: all .3s;
}
...
function parseDamageDie(die) {
  // e.g. "d6" -> {count:1, sides:6}  "2d6" -> {count:2, sides:6}
...
export default function CombatModal({
  ...
  // ── Phase ──────────────────────────────────────────────────────────────────
  const [phase, setPhase] = useState('initiative') // initiative | player_turn | rolling | damage_roll | enemy_turns | death_saves | flee_pick | fled | victory
  const [combatState, setCombatState] = useState(null)

  // ── Dice state ─────────────────────────────────────────────────────────────
  const [diceValues, setDiceValues] = useState([])    // current rolled values
  const [diceRolling, setDiceRolling] = useState([])  // which dice are animating
  const [diceConfig, setDiceConfig] = useState(null)  // {count, sides, label, isAdvantage}
  const [lockedDie, setLockedDie] = useState(null)    // for advantage: index of chosen die
  ...
  // ── Combat action state ────────────────────────────────────────────────────
  const [selectedAction, setSelectedAction] = useState(null) // 'attack'|'skills'|'flee'|'flee_roll'
  const [selectedTarget, setSelectedTarget] = useState(null) // enemy id
  const [selectedSkill, setSelectedSkill] = useState(null)
  const [isCrit, setIsCrit] = useState(false)
  const [attackResult, setAttackResult] = useState(null)
  const [showSkillsMenu, setShowSkillsMenu] = useState(false)
  const [fleeSkill, setFleeSkill] = useState(null)
  ...
  useEffect(() => {
    api.getCombatState(campaignId).then(s => {
  ...
  const addLog = (text, type = '') => {
    setLog(l => [...l, { text, type, id: Date.now() + Math.random() }])
  }
  ...
  const allies = combatState?.enemies?.filter(e => e.participant_type === 'ally') || []
  const round = combatState?.round_number || 1
  const isPlayerTurn = currentActor?.participant_type === 'player'
  const economy = combatState?.action_economy || {}
  ...
  const chooseInitiativeAdvantage = (idx) => {
    const chosen = initiativeAdvRolls[idx]
    setInitiativeRoll(chosen)
  }

  const confirmInitiative = async () => {
    if (initiativeRoll === null) return
    try {
      const result = await api.submitInitiative(
        campaignId,
        initiativeRoll,
  ...
  const selectTarget = (enemyId) => {
    if (selectedAction !== 'attack' && selectedAction !== 'skill_target') return
    ...
    startDiceRoll({
      ...
      onConfirm: async (rolls, chosen) => {
        ...
        try {
          ...
          if (result.hit) {
            if (result.is_crit) {
              addLog(`${player.name} attacks ${result.target_name} with ${result.weapon_name} — Critical Hit! (rolled ${roll})`, 'crit')
            } else {
              addLog(`${player.name} attacks ${result.target_name} with ${result.weapon_name} — Hit! (rolled ${roll + result.attack_bonus} vs AC ${result.enemy_ac})`, 'hit')
            }
  ...
  const confirmDamage = async (rolls, crit, targetName, weaponName) => {
    try {
      ...
      const total = result.damage_dealt

      addLog(
        `${player.name} deals ${total} damage to ${targetName} with ${weaponName}${result.enemy_dead ? ' — Enemy falls!' : ''}`,
  ...
  // ── Generic dice roller ────────────────────────────────────────────────────
  // diceConfig: {count, sides, label, isAdvantage, onConfirm}
  const [pendingDiceConfig, setPendingDiceConfig] = useState(null)

  const startDiceRoll = (config) => {
    const initial = Array.from({ length: config.count }, () => null)
  ...
  const rollAllDice = () => {
    if (!pendingDiceConfig) return
    const { count, sides } = pendingDiceConfig
    setDiceRolling(Array(count).fill(true))
    setTimeout(() => {
      const vals = Array.from({ length: count }, () => rollDie(sides))
      setDiceValues(vals)
      setDiceRolling(Array(count).fill(false))
    }, 350)
  }

  const confirmDiceRoll = () => {
    if (!pendingDiceConfig) return
    const { isAdvantage, onConfirm } = pendingDiceConfig
    const allRolled = diceValues.every(v => v !== null)
    if (!allRolled) return

    if (isAdvantage && diceValues.length === 2) {
      if (lockedDie === null) return // must pick one
      const chosen = diceValues[lockedDie]
      setPendingDiceConfig(null)
      setPhase('player_turn') // reset briefly, onConfirm may change it
      onConfirm(diceValues, chosen)
    } else {
      setPendingDiceConfig(null)
      setPhase('player_turn')
      onConfirm(diceValues, null)
    }
  }

  // ── Skills ─────────────────────────────────────────────────────────────────
  const combatAbilities = (abilities || []).filter(ab => {
    const name = ab.name.toLowerCase()
    return name.includes('second wind') || name.includes('action surge') ||
           name.includes('indomitable') || name.includes('maneuver') ||
           name.includes('psionic') || name.includes('rage') ||
           name.includes('cunning')
  })
  ...
  const useSkill = async (ability) => {
    ...
    if (name.includes('second wind')) {
      ...
      try {
        const result = await api.useCombatAbility(campaignId, 'action_surge')
        addLog(`${player.name} activates Action Surge!`, 'system')
        setActionUsed(false) // restore the action
  ...
  const endTurn = async () => {
    ...
    try {
      ...
      for (let i = 0; i < turnResults.length; i++) {
        ...
        const type = t.hit ? (t.damage > 0 ? 'hit' : 'system') : 'miss'
        addLog(t.text, type)
      ...
      // Check if it's player turn again
  ...
  const rollDeathSave = () => {
    startDiceRoll({
      ...
      onConfirm: async (rolls) => {
        ...
        if (nat20) {
          addLog(`${player.name} rolls a Natural 20 — stabilizes!`, 'heal')
          setDeathSuccesses(3)
          if (onPlayerUpdate) await onPlayerUpdate()
          setPhase('player_turn')
          return
  ...
  const endCombatVictory = async () => {
    ...
    setTimeout(() => onCombatEnd('victory', log), 800)
  }
      ...
      <div className="combat-overlay">
        <div className="combat-modal">
              ...
              {!hasAdvantage ? (
                ...
                <>
                  <div style={{ display: 'flex', gap: '1rem' }}>
                    {[0, 1].map(i => (
                      <div key={i}
                        ...
                        style={{ cursor: initiativeAdvRolls[i] !== null ? 'pointer' : 'pointer' }}
                  ...
                  {initiativeAdvRolls[0] !== null && (
                    <div className="initiative-sub" style={{ marginTop: 0 }}>
                      Click the die you want to use
                    </div>
                  )}
                  {initiativeRoll !== null && (
                    <div className="initiative-bonus">
              ...
              <div className="death-save-dots">
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '.4rem' }}>
                    {[0,1,2].map(i => (
                      <div key={i} className={`ds-pip${i < deathSuccesses ? ' success' : ''}`} />
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '.4rem' }}>
                    {[0,1,2].map(i => (
                      <div key={i} className={`ds-pip${i < deathFailures ? ' failure' : ''}`} />
                    ))}
                  </div>
                </div>
              </div>
              <button className="btn-gold" onClick={rollDeathSave}>
                Roll Death Save
              </button>
            </div>
          )}
              ...
              <div className="enemy-row">
                {enemies.map(enemy => (
                  <div
                    ...
                    className={[
                      ...
                      attackingEnemy === enemy.id ? 'attack-out' : '',
                      (!canAct || !selectedAction) ? 'disabled' : ''
                    ].filter(Boolean).join(' ')}
                ...
                {allies.map(ally => (
                  <div key={ally.id} className={[
                    'enemy-card',
                    !ally.is_alive ? 'dead' : '',
                  ].filter(Boolean).join(' ')} style={{ cursor: 'default' }}>
                    <div className="enemy-icon">
                      <EnemyIcon isAlly={true} />
                    </div>
                    <div className="enemy-name" style={{ color: 'var(--grn)' }}>{ally.name}</div>
                  </div>
                ))}
              </div>

              {/* Player / Companion Row */}
              ...
              <div className="dice-display">
                {diceValues.map((val, i) => (
                  <div
                    ...
                    onClick={() => {
                      if (pendingDiceConfig.isAdvantage && val !== null) {
                        setLockedDie(i)
                      } else {
                        rollSingleDie(i)
                      }
                ...
                {diceValues.some(v => v === null) && (
                  <button className="dice-confirm-btn" onClick={rollAllDice}>
                    Roll All
                  </button>
                )}
                  ...
                  <>
                    {pendingDiceConfig.isAdvantage && (
                      <span className="dice-advantage-note">
                        {lockedDie !== null
                          ? `Using ${diceValues[lockedDie]}`
                          : 'Click the die to use'}
                      </span>
                    )}
                    <button
          ...
          {phase === 'player_turn' && !selectedTarget && selectedAction !== 'flee' && (
            <div className="action-economy">
              ...
              <div className="economy-slot">
                ...
                <div className="economy-buttons" style={{ position: 'relative' }}>
                  <button
                  ...
                  <button className="action-btn" disabled={true} title="Coming soon">
                    ✧ Spells
                  </button>
              ...
              <div className="economy-slot">
                ...
                <div className="economy-buttons">
                  <button
                    className={`action-btn${bonusActionUsed ? ' used' : ''}`}
          ...
          <div className="combat-log" ref={logRef}>
            {log.length === 0 && (
              <div className="log-entry system">Combat begins…</div>
            )}
            {log.map(entry => (

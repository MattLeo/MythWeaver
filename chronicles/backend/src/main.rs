...
pub async fn get_available_feats_handler(
    ...
    let armor_training: Vec<&str> = profs.iter()
        .filter(|pr| pr["proficiency_type"].as_str() == Some("armor"))
        .filter_map(|pr| pr["name"].as_str())
        .collect();
    ...
    match feats_db::get_available_feats(
        pool, &p.id, p.level,
        p.subclass.as_deref(),
        has_spellcasting, has_fighting_style, &armor_training,
